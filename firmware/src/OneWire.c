 ...
 *  - standard-speed (accurate 1�s delay)
...
uint8_t OneWire_ResetBus(void)
{
  ...
  /*
   *  pull down DQ for >=480�s and release it again
   */
   ...
   *  - time slot >=480�s
   *  - client responds after 15-60�s with a low pulse of 60-240�s
   *  - we check DQ after 70�s
   */

  wait50us();                 /* read delay of 70�s */
  wait20us();
  ...
  /* end the time slot */
  wait400us();                /* delay of 430�s (500�s - 70�s) */
  wait30us();
...
{
  /*
   *  - write time slot: 60-120�s
   *  - recovery time after write: >=1�s
   *  - write "1":
   *    - 1-15�s low pulse
   *    - release DQ and wait until end of time slot
   *  - write "0":
   *    - 60-120�s low pulse (complete time slot)
   *    - release DQ
   */
  ...
  {
    /*
     *  pull down DQ for 5�s and release it again
     */

    wait5us();                /* pulse delay of 5�s */
  ...
  {
    /*
     *  pull down DQ for 60�s and release it again
     */

    wait50us();               /* pulse delay of 60�s */
    wait10us();
...
uint8_t OneWire_ReadBit(void)
{
  ...
  /*
   *  - read time slot: 60-120�s
   *  - recovery time after read: >=1�s
   *  - master starts read process by a low pulse >=1�s
   *  - client's response is valid for 15�s after falling edge of
   *    the master's low pulse
   *  - we read DQ after 13�s
   */
   ...
   *  - data bit valid for 15�s starting with master's read pulse
   *  - high -> 1
   *  - low  -> 0
   */

  wait5us();                /* read delay of 8�s */
  wait3us();
