...
uint8_t MeasureInductance(uint32_t *Time, uint8_t Mode)
{
  ...
  {
    Test = MCU_CYCLES_PER_US;           /* MCU cycles per �s */
...
uint8_t MeasureInductor(Resistor_Type *Resistor)
{
  ...
  {
    /* a valid time should be larger than the delay (4�s) */
    if (Time1 <= 5000)         /* <= 5�s */
    {
      Test = 0;                /* invalid time */
    }
  ...
  if (Test == 3)                   /* valid measurement */
  {
    ...
    if (Mode & MODE_LOW_CURRENT)   /* low current measurement mode */
    {
      ...
      #if CPU_FREQ == 8000000
      /* 8 MHz */
      if (Time1 < 6000)       /* < 6�s / < 18mH */
      {
        Offset = 38;
      }
      else if (Time1 < 9500) /* 6-9.5�s / 18-27mH */
      {
        Offset = -10;
      }
      else                    /* > 9.5�s / > 27mH */
      ...
      else if (Time1 < 12500) /* 6-12.5�s / 18-33mH */
      {
        Offset = 45;
      }
      else                    /* > 12.5�s / > 33mH */
    ...
    else                           /* high current measurement mode */
    {
      ...
      Temp = (uint16_t)Time1;      /* time < 50�s expected */
