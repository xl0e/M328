...
/* important constants */
#define IR_SAMPLE_PERIOD         50     /* 50 �s */
...
void IR_Decode(uint8_t *PulseData, uint8_t Pulses)
{
   ...
   *  - PDM: pulse 560�s, pause 0=560�s 1=1690�s
   *  - bit mode: LSB
   *  - stop: pulse 560�s
   *  - standard format: 
  ...
  if (PulseCheck(Time1, 179, IR_RELAX_LONG))      /* pulse 9ms */
  {
    ...
    else if (PulseCheck(Time2, 45, IR_STD_TOLER))      /* pause 2.25ms */
    {
      ...
      {
        if (PulseCheck(*Pulse, 11, IR_STD_TOLER))      /* pulse 560�s */
        {
          Display_NL_EEString_Space(IR_NEC_str);       /* display protocol */
   ...
   *  - PDM: pulse 500�s, pause 0=500�s 1=1500�s
   *  - bit mode: LSB
   *  - stop: pulse 500�s
   ...
   *  - PDM: pulse 525�s, pause 0=525�s 1=1575�s
   *  - bit mode: LSB
   *  - stop: pulse 525�s
   ...
   *      PDM: pulse 560�s, pause 0:600�s 1:1620�s 
   *    - start: pulse 9ms, pause 4.2ms (180/84)
   *      PDM: pulse 550�s, pause 0:550�s 1:1580�s
   */
   ...
   *  - PDM: pulse 872�s, pause 0=872�s 1=2616�s
   *  - bit mode: LSB
   *  - stop: pulse 872�s
   *  - format 12 bits / C6D6:
   ...
   *  - start: pulse 3456�s, pause 1728�s
   *  - PDM: pulse 432�s, pause 0=432�s 1=1296�s
   *  - bit mode: LSB
   *  - stop: pulse 432�s
   *  - format (48 bits):
   ...
   *  - repeat sequence format: <pulse 3456�s><pause 3456�s><pulse 432�s>
  ...
  if (PulseCheck(Time1, 70, IR_STD_TOLER))        /* pulse 3.5ms */
  {
    ...
    /* Kaseikyo */
    if (PulseCheck(Time2, 34, IR_STD_TOLER))      /* pause 1728�s */
    {
   ...
   *  - PDM: pulse 500�s, pause 0=1ms 1=2ms
   *  - stop: pulse 500�s
   *  - bit mode: MSB
   ...
   *    - PDM: pulse 560�s, pause 0:1ms 1:2ms
  ...
  /*
   *  Motorola
   *  - start: pulse 512�s, pause 2560�s
   *  - Bi-Phase (Thomas):
   *    0: pause 512�s, pulse 512�s
   *    1: pulse 512�s, pause 512�s 
   *  - bit mode: LSB
   ...
   *  - start: pulse 550�s, pause 2.5ms
   *  - Bi-Phase (Thomas):
   *    0: pause 550�s, pulse 550�s
   *    1: pulse 550�s, pause 550�s
   *  - bit mode: LSB
   *  - a valid code consists of a start packet, one or more command packets
   *    and an end packet
  ...
  if (PulseCheck(Time1, 11, IR_STD_TOLER))        /* pulse 512�s */
  {
    if (PulseCheck(Time2, 52, IR_STD_TOLER))      /* pause 2560�s */
    {
   ...
   *  Thomson
   *  - no start / AGC burst
   *  - PDM: pulse 500�s, pause 0:2ms 1:4.5ms
   *  - stop: pulse 500�s
   *  - bit mode: LSB
   ...
   *  - alternative timing:
   *    PDM: pulse 470�s, pause 0:2.05ms 1:4.6ms
   *    stop: pulse 470�s
   */

  #ifdef SW_IR_RX_EXTRA
  if (PulseCheck(Time1, 10, IR_STD_TOLER))        /* pulse 500�s */
  {
  ...
  /*
   *  Sony SIRC
   *  - start: pulse 2.4ms (pause 600�s)
   *  - PWM: pause 600�s, pulse 0=600�s 1=1200�s
   *  - bit mode: LSB
  ...
  {
    if (PulseCheck(Time2, 12, IR_STD_TOLER))      /* pause 600�s */
    {
      Display_NextLine();               /* new line */
   ...
   *  - no start / AGC burst
   *  - PDM: pulse 140.8�s, pause 0:5060�s 1:7590ms
   *  - stop: pulse 140.8�s
   *  - bit mode: MSB
   ...
   *  - alternative timing:
   *    PDM: pulse 180�s, pause 0:4850�s 1:7.4ms
   *  - carrier 38kHz (455kHz/12), duty cycle 1/3
   */

  /*
   *  RECS80 Extended (SAA3008)
   *  - start: pulse 140.8�s, pause 3659.2�s, pulse 140.8�s, pause 3659.2�s
   *  - PDM: pulse 140.8�s, pause 0=4919.2�s 1=7459.2�s
   *  - bit mode: MSB
   *  - stop: pulse 140.8�s
  ...
  #ifdef SW_IR_RX_EXTRA
  if (PulseCheck(Time1, 4, IR_STD_TOLER))         /* pulse 141�s */
  {
    ...
    else if (PulseCheck(Time2, 173, IR_RELAX_LONG))  /* pause 3659�s */
    {
   ...
   *  - also: Denon
   *  - no start / AGC burst
   *  - PDM: pulse 264�s, pause 0=786�s 1=1836�s
   *  - bit mode: LSB
   *  - stop: pulse 320�s
   ...
   *  - alternative timing:
   *    - pulse 320�s, pause 0=680�s 1=1680�s
   */

  if (PulseCheck(Time1, 6, IR_STD_TOLER))    /* pulse 320�s */
  {
    /* pause 680�s or 1680�s */
   ...
   *  - 2 start bits: (889�s L) 889�s H, 889�s L (889�s H)
   *  - Bi-Phase (IEEE 802.3):
   *    0: pulse 889�s, pause 889�s
   *    1: pause 889�s, pulse 889�s
   *  - bit mode: MSB
   ...
   *  - 2 start bits: (889�s L) 889�s H + 889�s H (889�s L)
   *  - Bi-Phase (IEEE 802.3):
   *    0: pulse 889�s, pause 889�s
   *    1: pause 889�s, pulse 889�s
   *  - bit mode: MSB
  ...
  if (PulseCheck(Time1, 17, IR_STD_TOLER))        /* pulse 889�s */
  {
    if (PulseCheck(Time2, 17, IR_STD_TOLER))      /* pause 889�s */
    {
   ...
   *  - alternative timing:
   *    PPM: 0:pause 1:pulse 1590�s
   *    packet delay 51,2ms (start to start)
   *    carrier 26.7kHz (320kHz/12)
   */

  #ifdef SW_IR_RX_EXTRA
  if (PulseCheck(Time1, 22, IR_STD_TOLER))        /* pulse 1120�s (for 1) */
  {
    if (PulseCheck(Time2, 22, IR_STD_TOLER))      /* pause 1120�s (for 0) */
    {
      /* try to demodulate */
   ...
   *  - start: pulse 2664�s, pause 888�s
   *  - Bi-Phase (Thomas):
   *    normal bit 0: pause 444�s, pulse 444�s
   *    normal bit 1: pulse 444�s, pause 444�s
   *    toggle bit 0: pause 888�s, pulse 888�s
   *    toggle bit 1: pulse 888�s, pause 888�s
   *  - bit mode: MSB
  ...
  if (PulseCheck(Time1, 53, IR_STD_TOLER))        /* pulse 2664�s */
  {
    if (PulseCheck(Time2, 17, IR_STD_TOLER))      /* pause 888�s */
    {
...
void IR_Detector(void)
{
  ...
  /*
   *  adaptive sampling delay for 10�s considering processing loop
   *  - processing loop needs about 24 MCU cycles (3�s@8MHz)
   */

  Cycles = MCU_CYCLES_PER_US;           /* MCU cycles per �s */
  Cycles *= 10;                         /* cycles for 10�s */
   ...
   *  we sample IR data every 50�s and log how long each pulse or pause
  ...
  while (Run > 0)             /* processing loop */
  {
    ...
    if (Run == MODE_SAMPLE)             /* sampling mode */
    {
      ...
      /*
       *  adaptive delay for 10�s considering processing loop
       *  - a loop run needs 4 cycles, the last loop run just 3
       *  - time delay: 10�s - 1 MCU cycle
       */
