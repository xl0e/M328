 ...
 *
 *  requires:
 *  - Timeout: timeout (in 10�s steps)
 *
 *  returns:
 *  - 0  error or timeout exceeded
 *  - >0 time (in 10�s steps)
 */
...
{
  uint8_t           Flag = 0;      /* return value */
  uint8_t           Ticks;         /* time ticks (in 10�s) */
  ...
  {  
     ...
     *  - 20-40�s after MCU released Data line
     *  - pulls down Data line for 80�s
     *  - releases Data line for 80�s
     */

    /* wait up to 40�s for sensor to pull down DATA line */
    Ticks = DHTxx_WaitPullDown(5);

    if (Ticks > 0)                 /* time ok */
    {
      /* wait about 80�s for sensor to release DATA line */
      Ticks = DHTxx_WaitRelease(9);

      if (Ticks >= 6)              /* time ok */
      {
        /* wait about 80�s for sensor to pull down DATA line again */
   ...
   *  - each bit starts with pull-down of Data line for 50�s
   *  - 0 is indicated by release of Data for 26-28�s
   *  - 1 is indicated by release of Data for 70�s
   *  - after the last bit Data line is pulled down for 50�s
   *  - bit order: MSB
   */
  ...
  while (Bytes > 0)                /* 5 bytes */
  {
    ...
    while (Bits > 0)               /* 8 bits */
    {
      /* wait about 50�s for sensor to release Data line */
      Ticks = DHTxx_WaitRelease(6);

      if (Ticks >= 4)              /* time ok */
      {
        /* wait up to 70�s for sensor to pull down Data line again */
        ...
        else if ((Ticks >= 1) && (Ticks <= 3))    /* 26-28�s -> 0 */
        {
          /* clear bit */
          Byte <<= 1;              /* shift one bit left */
        }
 ...
 *  - byte 3: temperature high byte (integer part, in �C)
 ...
 *  - T_High: temperature high byte (in �C)
 */
 ...
 *  - byte 3: temperature high byte (in 0.1 �C)
 ...
 *  - T_High: temperature high byte (in 0.1 �C)
